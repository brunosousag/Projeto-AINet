<x-layouts::main-content title="Catalog"
    ...
    <div class="space-y-6">
        <form method="GET" action="{{ route('shop.index') }}"
            ...
            <div class="flex items-end">
                <flux:button :href="route('shop.index')" icon="x-mark" variant="ghost">Reset</flux:button>
            </div>
        </form>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                @foreach ($tshirtImages as $tshirtImage)
                    <article class="overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                        <a href="{{ route('shop.tshirt-details', ['tshirtImage' => $tshirtImage]) }}" class="block bg-zinc-100 p-5 dark:bg-zinc-800">
                        ...
                        <div class="space-y-4 p-4">
                            <div>
                                <div class="flex items-start justify-between gap-3">
                                    <h2 class="text-base font-semibold text-zinc-900 dark:text-zinc-100">
                                        <a href="{{ route('shop.tshirt-details', ['tshirtImage' => $tshirtImage]) }}">
                                            {{ $tshirtImage->name }}
                                        </a>
                                    </h2>
                            ...
                            <flux:button :href="route('shop.tshirt-details', ['tshirtImage' => $tshirtImage])"
                                         icon="eye"
                                         variant="primary"
                                         class="w-full">
                                Choose t-shirt
