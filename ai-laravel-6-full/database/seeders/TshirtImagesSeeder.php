...
class TshirtImagesSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $this->cleanStorageFolder('tshirt_images');
        $this->cleanStorageFolder('tshirt_images_private', false);
        foreach ($this->imagens_tshirtImage_set1 as $info_tshirtImage) {
            $new_tshirtImage = $this->newTshirtImage($faker, $info_tshirtImage[0], $info_tshirtImage[1],
                DatabaseSeeder::$seedLanguage == 'pt_PT' ? $info_tshirtImage[3] : $info_tshirtImage[2]);
    ...
    {
        $folder = $publico ? 'tshirtImages_catalog' : 'tshirtImages_own';
        $targetfolder = $publico ? 'tshirt_images' : 'tshirt_images_private';
        $newFileName = $this->copyFileToStorage($folder, $file, $targetfolder, null, $id, $publico);
